...
namespace ZoomLa.BLL
{
    public class WxAPI
    {
        ...
        {
            if (appMod == null || appMod.ID < 1) { return null; }
        ...
        //对错误信息进行处理后提示
        public void ErrorMsg(string result)
        {
            if (string.IsNullOrEmpty(result)) { throw new Exception("返回值为空"); }
            JObject obj = JsonConvert.DeserializeObject<JObject>(result);
            if (obj["errcode"] == null) { throw new Exception(result); }
        ...
        public bool IsSuccess(string result)
        {
            bool flag = false;
            if (string.IsNullOrEmpty(result)) { return flag; }
            try
            {
                JObject json = JsonConvert.DeserializeObject<JObject>(result);
                if (json["errcode"] == null || json["errcode"].ToString().Equals("0")) { flag = true; }
            }
            catch { flag = false; }
            return flag;
        }
        ...
        public string GetToken()
        {
            ...
            if (!IsSuccess(result)) { ErrorMsg(result); }
        ...
        public string SendAll(M_WXAllMsg model, bool flag = true)
        {
            ...
            if (!IsSuccess(result) && flag) { ErrorMsg(result); }
            return result;
        }
        /// <summary>
        /// 单个发送文本消息,以避开群发,人数多应该单开线程,每200毫秒发送一条
        /// 多图文必须使用群发接口(否则内容点不进去,缩略图无法正常显示)
        /// </summary>
        ...
        private void AddMpNewsForLocal(string media_id, string subnews)
        {
            ...
            newsobj.Add(new JProperty("update_time", DateHelper.DateToUnix(DateTime.Now)));
        ...
        public string DelWxMaterial(string media_id, string type)
        {
            ...
            if (IsSuccess(result)) { DelWxMaterialByLocal(media_id, type); }
            return result;
        }
        /// <summary>
        /// 删除存储在本地微信素材
        ...
        /// </summary>
        /// <param name="media">返回的媒体ID,如果上传失败则存储失败原因</param>
        public bool UploadImg(string vpath, ref string media)
        {
            string ppath = function.VToP(vpath);
            if (!File.Exists(ppath)) { media = "[" + vpath + "]文件不存在"; return false; }
            try
            {
                FileInfo file = new FileInfo(ppath);
                FileStream fs = file.Open(FileMode.Open, FileAccess.Read);
                JObject imgobj = UploadImg(fs, Path.GetFileName(vpath));
                media = imgobj["media_id"].ToString();
            }
            catch (Exception ex) { media = ex.Message; return false; }
            return true;
        }
        /// <summary>
        /// 上传永久图片素材,返回媒体ID给群发消息用
        /// </summary>
        public JObject UploadImg(Stream stream, string fname)
        {
            M_Http_File fileMod = new M_Http_File(IOHelper.StreamToBytes(stream), fname);
            string url = apiurl + "material/add_material?access_token=" + AccessToken + "&type=image";
            M_Http_Request req = HttpHelper.UploadSignleFile(url, "media", fileMod);
            JObject imgobj = JsonConvert.DeserializeObject<JObject>(req.result.resultStr);
            if (IsSuccess(req.result.resultStr))
            {
                AddImgMaterialByLocal(imgobj["media_id"].ToString(), imgobj["url"].ToString());
            }
            else
            {
                ErrorMsg(req.result.resultStr);
            }
            return imgobj;
        }
        /// <summary>
        ...
        /// </summary>
        public static void AutoSync(HttpContext ctx, string url, M_WX_APPID appMod = null)
        {
            ...
            //        string redirect_uri = SiteConfig.SiteInfo.SiteUrl + "/Bu/wxuser";
        ...
        public bool IsFollow(int uid)
        {
            ...
            if (appMod == null || string.IsNullOrEmpty(appMod.OpenID)) { return false; }
            ...
            if (jobj["subscribe"] != null && jobj["subscribe"].ToString() == "1") { return true; }
            else { return false; }
        }
    }
}