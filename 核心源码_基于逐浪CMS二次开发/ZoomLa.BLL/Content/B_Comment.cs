...
namespace ZoomLa.BLL
{
    ...
    public class B_Comment
    {
        ...
        /// 用于CommentFor
        /// </summary>
        public PageSetting SeachCommentByGid1(int cpage, int psize, int gid)
        {
            string fields = "A.*,B.UserName,B.Salt AS UserFace,";
            ...
            string where = "GeneralID=" + gid + " AND Audited=1 ";
            //string order = PK + " DESC";
            PageSetting setting = PageSetting.Double(cpage, psize, TbName, "ZL_User", "A." + PK, "A.UserID=B.UserID", where, "", null, fields);
            DBCenter.SelPage(setting);
            return setting;
        ...
        {
            string fields = "A.*,B.Title AS DocTitle,B.HtmlLink,";
            //同意,反对,举报
            fields += "(SELECT COUNT(*) FROM ZL_Comment_Count Comm WHERE A.CommentID=Comm.CommentID AND Comm.status=1) AS Agree,";
            fields += "(SELECT COUNT(*) FROM ZL_Comment_Count Comm WHERE A.CommentID=Comm.CommentID AND Comm.status=-1) AS DisAgree";
            return SqlHelper.JoinQuery(fields, "ZL_Comment", "ZL_CommonModel", "A.GeneralID=B.GeneralID"
        ...
        public int GetComment_CountIDTitlePK(int generalID, bool PKS)
        {
            ...
            return DataConvert.CLng(SqlHelper.ExecuteScalar(CommandType.Text, sqlStr));
        }
        /// <summary>
        /// 查询某节点的是否审核通过的评论
        /// </summary>
