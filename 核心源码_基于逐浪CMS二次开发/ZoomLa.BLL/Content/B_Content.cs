...
using static ZoomLa.Model.ZLEnum;
...
namespace ZoomLa.BLL.Content
{
    public class B_Content
    {
        ...
        public M_CommonData SelModelByTitle(string title, int nid = 0)
        {
            if (string.IsNullOrEmpty(title)) { return null; }
            SqlParameter[] sp = new SqlParameter[] { new SqlParameter("title", title.Trim()) };
            string where = " Title=@title";
            if (nid > 0) { where += " AND NodeID=" + nid; }
            return SelReturnModel(where, sp);
        }
        ...
        public DataTable GetByDupTitle(string title)
        {
            ...
            List<SqlParameter> sp = new List<SqlParameter>() { new SqlParameter("title", "%" + title + "%") };
            dt = DBCenter.SelWithField(TbName, "GeneralID,Title", "TableName Like 'ZL_C_%' AND Title Like @title", "", sp);
            return dt;
        }
        ...
        //--------------------------------------------------用于ContentManage
        public PageSetting SelPage(int cpage, int psize, Filter_Content filter)
        {
            List<SqlParameter> spList = new List<SqlParameter>();
            string where = "", order = StrHelper.SQL_OrderBy(filter.orderBy, "generalid,elitelevel,hits,istop", "A.OrderID Desc,GeneralID Desc");
            where = GetPageWhere(spList, filter.NodeID, filter.ModelID, filter.Status, filter.KeyType, filter.KeyWord, filter.authNodes);
            if (filter.special > 0) { where += " AND SpecialID LIKE '%," + filter.special + ",%'"; }
            ////用于计算点击数等
        ...
        {
            return SelPage(pageIndex, pageSize, new Filter_Content()
            {
                NodeID = nodeID,
                ModelID = modelID,
                Status = status,
        ...
        //------------
        public PageSetting SelContent(int cpage, int psize, int nodeid, string status, string inputer, string skey, string tbname = "")
        {
            string where = "";
            ...
            {
                where += " AND TableName LIKE @tbname";
                sp.Add(new SqlParameter("tbname", "%" + tbname + "%"));
            }
        ...
        public void UpdateElite(string ids, int elite)
        {
            SafeSC.CheckIDSEx(ids);
            DBCenter.UpdateSQL(TbName, "EliteLevel=" + elite, "GeneralID IN (" + ids + ")");
        }
        ...
        public DataTable GetContentRecycle(int NodeID)
        {
            ...
            return DBCenter.JoinQuery("A.*,B.NodeName", TbName, "ZL_Node", "A.NodeID=B.NodeID", where, "GeneralID DESC");
        }
        #region 黄页相关
        ...
        public PageSetting Page_GetRecycle_SelPage(int cpage, int psize, string skey = "", string uname = "")
        {
            string where = "Status=-2 And tablename like 'ZL_Page_%'";
            List<SqlParameter> sp = new List<SqlParameter>();
            ...
            {
                where += " AND Inputer=@uname";
                sp.Add(new SqlParameter("uname", uname));
            }
            PageSetting setting = PageSetting.Single(cpage, psize, TbName, PK, where, "GeneralID DESC", sp);
            DBCenter.SelPage(setting);
            return setting;
        }
        ...
        public PageSetting Page_Sel(int cpage, int psize, int nodeid = 0, string status = "", string inputer = "", string skey = "")
        {
            string where = " A.TableName LIKE '%ZL_Page_%' ";
            List<SqlParameter> sp = new List<SqlParameter>();
            if (status.Equals("-100")) { }//全部|除回收站|指定状态
            else if (string.IsNullOrEmpty(status)) { where += " AND A.Status!=" + (int)ZLEnum.ConStatus.Recycle + " "; }
            else { SafeSC.CheckIDSEx(status); where += " AND A.Status IN (" + status + ") "; }
            if (!string.IsNullOrEmpty(skey)) { where += " AND A.Title LIKE @skey"; sp.Add(new SqlParameter("skey", "%" + skey + "%")); }
            if (!string.IsNullOrEmpty(inputer)) { where += " AND A.Inputer=@inputer"; sp.Add(new SqlParameter("inputer", inputer)); }
            if (nodeid > 0) { where += " AND A.NodeID=" + nodeid; }
            string stable = "(SELECT A.*,B.ItemName,B.ModelName FROM ZL_CommonModel A LEFT JOIN ZL_Model B ON A.ModelID=B.ModelID)";
            PageSetting setting = PageSetting.Double(cpage, psize, stable, "ZL_PageReg", "A.GeneralID", "A.Inputer=B.UserName", where, "A.GeneralID DESC", sp, "A.*,B.ID AS RegID,B.UserID");
            DBCenter.SelPage(setting);
            return setting;
        }
        #endregion
        ...
        /// 获取我的店铺,如未通过则报错
        /// </summary>
        public M_CommonData SelMyStore_Ex(HttpContext ctx, ref string err)
        {
            M_CommonData storeMod = SelMyStore(new B_User(ctx).GetLogin().UserName);
            if (storeMod == null) { err = "你尚未开通店铺"; }
            else if (storeMod.Status != (int)ConStatus.Audited) { err = "你的店铺尚未通过审核"; }
            return storeMod;
        }
        ...
        public M_CommonData Store_SelModel(int shopid)
        {
            ...
            conMod = SelReturnModel(shopid);
            return conMod;
        }
        public DataTable Store_Sel(string skey = "", bool hasshop = true)
        {
            string where = "Tablename='ZL_Store_reg' ";
            List<SqlParameter> sp = new List<SqlParameter>() { };
            if (!string.IsNullOrEmpty(skey)) { where += " AND Title LIKE @skey"; sp.Add(new SqlParameter("skey", "%" + skey + "%")); }
        ...
        public void UpdateContent(DataTable ContentDT, M_CommonData model)
        {
            if (ContentDT != null && ContentDT.Rows.Count > 0)
            {
                ...
                if (model.ItemID > 0 && DBCenter.IsExist(model.TableName, "ID=" + model.ItemID))
                {
                    DBCenter.UpdateSQL(model.TableName, BLLCommon.GetFieldAndPara(ContentDT), "ID=" + model.ItemID, splist);
                }
                else
                {
                    model.ItemID = DBCenter.Insert(model.TableName, BLLCommon.GetFields(ContentDT), BLLCommon.GetParas(ContentDT), splist.ToArray());
                }
            }
            UpdateByID(model);
        }
        ...
        public DataTable GetContent(M_CommonData model)
        {
        ...
        public bool MoveContent(string ContentIDS, int NodeID)
        {
            ...
            int firstNodeID = nodeBll.SelFirstNodeID(NodeID);
            return DBCenter.UpdateSQL(TbName, "NodeID=" + NodeID + ",ParentTree='" + tree + "',FirstNodeID=" + firstNodeID + "", "GeneralID IN (" + ContentIDS + ")");
        }
        /// <summary>
        /// 移入回收站
        /// </summary>
        ...
        public PageSetting SelPageByTagkey(int cpage, int psize, string tagkey = "", string key = "", int status = -100)
        {
            string where = "1=1";
            List<SqlParameter> sp = new List<SqlParameter>();
            if (!string.IsNullOrEmpty(tagkey)) { where += " AND Tagkey LIKE @tagkey"; sp.Add(new SqlParameter("tagkey", "%" + tagkey + "%")); }
            if (!string.IsNullOrEmpty(key)) { where += " AND Title LIKE @title"; sp.Add(new SqlParameter("title", "%" + key + "%")); }
            if (status != -100) { where += " AND Status = " + status; }
            PageSetting setting = PageSetting.Double(cpage, psize, TbName, "ZL_C_Article", "A." + PK, "A.ItemID=B.ID", where, "", sp);
            DBCenter.SelPage(setting);
            return setting;
        }
        #region Tools
