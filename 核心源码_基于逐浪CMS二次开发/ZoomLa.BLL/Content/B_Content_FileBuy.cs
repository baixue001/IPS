...
namespace ZoomLa.BLL.Content
{
    public class B_Content_FileBuy : ZL_Bll_InterFace<M_Content_FileBuy>
    {
        ...
        public PageSetting SelPage(int cpage, int psize, int uid)
        {
            ...
            PageSetting setting = PageSetting.Double(cpage, psize, TbName, "ZL_CommonModel", "A." + PK, "A.Gid=B.GeneralID", where, "", null, "A.*,B.Title");
        ...
        public M_Content_FileBuy SelByGid(int uid,int gid, string ranstr)
        {
            ...
            DbDataReader reader = SqlHelper.ExecuteReader(CommandType.Text, sql, sp);
            if (reader.Read())
                return initMod.GetModelFromReader(reader);
            else
                return null;
