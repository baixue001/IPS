...
namespace ZoomLa.BLL.Content
{
    public class B_DocPermission
    {
        ...
        public M_DocPermission SelByUserID(string userID)
        {
            ...
            using (DbDataReader reader = Sql.SelReturnReader(strTableName, " Where id = (select OwnGroupID from ZL_DocPermission Where UserID = @userID)", sp))
            {
                if (reader.Read())
                {
        ...
        {
            using (DbDataReader reader = Sql.SelReturnReader(strTableName, " Where id = 1"))
            {
                if (reader.Read())
                {
                    return initmod.GetModelFromReader(reader);
