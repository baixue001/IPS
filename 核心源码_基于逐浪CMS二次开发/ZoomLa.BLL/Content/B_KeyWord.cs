...
namespace ZoomLa.BLL
{
    ...
    public class B_KeyWord
    {
        ...
        public void AddKeyWord(string tagkey, int type)
        {
            ...
            string[] keyarray = tagkey.Split(",".ToCharArray(), StringSplitOptions.RemoveEmptyEntries)
                .Distinct().ToArray();//去除重复项
            ...
            {
                string name = "@key" + i;
                string value = keyarray[i].Replace(" ", "").Replace(",", "");
                if (string.IsNullOrEmpty(value)) { continue; }
                paras += name + ",";
                sp.Add(new SqlParameter(name, value));
            }
            paras = paras.TrimEnd(',');
            //-----------------------------------
            string sql = "SELECT * FROM ZL_Keywords WHERE KeywordText IN (" + paras + ")";
            DataTable dt = SqlHelper.ExecuteTable(sql, sp.ToArray());
            foreach (SqlParameter item in sp)
            {
                if (dt.Select("KeywordText='" + item.Value + "'").Length < 1)
                {
                    insert(new M_KeyWord() { KeywordText = item.Value.ToString(), KeywordType = type });
                }
            }
        }
        ...
        public int GetNameListTotal(string keyword)
        {
            ...
            return DataConvert.CLng(SqlHelper.ExecuteScalar(CommandType.Text, strSql, cmdParam));
        }
    }
}