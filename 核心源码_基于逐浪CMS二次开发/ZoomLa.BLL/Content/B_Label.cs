...
namespace ZoomLa.BLL
{
    public class B_Label
    {
        ...
        {
            if (string.IsNullOrEmpty(label.LabelName)) { label.LabelName = "随机" + DateTime.Now.ToString("yyyyMMdd") + "_" + function.GetRandomString(6); }
            ...
            string filename = dir + label.LabelCate + @"\" + label.LabelName + ".label";
            ExistLabelDir(label);
            if (directoryDT.Columns.Count > 0)
            {
                directoryDT.DefaultView.RowFilter = "LabelName='" + label.LabelName + "'";
            }
            directoryDT = directoryDT.DefaultView.ToTable();
            if (directoryDT.Rows.Count > 0)
            {
                function.WriteErrMsg("已存在该标签[" + label.LabelName + "],请使用其他表签名!");
            }
            directoryDT.Dispose();
            ...
            rows["LabelName"] = label.LabelName;
            rows["LabelType"] = label.LabelType;
        ...
        {
            if (string.IsNullOrEmpty(label.LabelName)) { label.LabelName = "随机命名" + DateTime.Now.ToString("yyyyMMdd") + "_" + function.GetRandomString(6); }
            ExistLabelDir(label);
            XmlDocument doc = new XmlDocument();
            doc.Load(dirfilename);
            XmlNode nodelist = doc.SelectSingleNode("//NewDataSet/Table[LabelName='" + label.LabelName + "']");
            if (nodelist == null) { throw (new Exception("[" + label.LabelName + "]不存在")); }
            string oldPath = (nodelist["LabelCate"].InnerText + "\\" + nodelist["LabelName"].InnerText).Replace(" ", "").ToLower();
            string newPath = (label.LabelCate + "\\" + label.LabelName).ToLower();
            //--------------------
            DataTable labelDT = GetInfoFromModel(label);
            ...
            if (dsDT.Columns.Count > 0) { dsDT.DefaultView.RowFilter = "LabelName='" + label.LabelName + "'"; }
            ...
            nodelist["LabelName"].InnerText = label.LabelName;
            nodelist["LabelCate"].InnerText = label.LabelCate;
            nodelist["LabelType"].InnerText = label.LabelType.ToString();
        ...
        private DataTable GetLabelStruct(M_Label labelMod = null)
        {
            ...
            table.Columns.Add(new DataColumn("EmptyContent", typeof(string)));
            ...
            if (labelMod != null)
            {
                ...
                row["LabelID"] = labelMod.LabelID;
                row["LabelName"] = labelMod.LabelName;
                row["LabelType"] = DataConverter.CLng(labelMod.LabelType);
                row["LabelCate"] = labelMod.LabelCate;
                row["LabelDesc"] = labelMod.Desc;
                row["LabelParam"] = labelMod.Param;
                row["LabelTable"] = labelMod.LabelTable;
                row["LabelField"] = labelMod.LabelField;
                row["LabelWhere"] = labelMod.LabelWhere;
                row["LabelOrder"] = labelMod.LabelOrder;
                row["LabelContent"] = labelMod.Content;
                row["LabelCount"] = labelMod.LabelCount;
                row["LabelAddUser"] = DataConverter.CLng(labelMod.LabelAddUser);
                row["LabelNodeID"] = DataConverter.CLng(labelMod.LabelNodeID);
                row["Modeltypeinfo"] = labelMod.Modeltypeinfo;
                row["addroot"] = labelMod.addroot;
                row["setroot"] = labelMod.setroot;
                row["Modelvalue"] = labelMod.Modelvalue;
                row["Valueroot"] = labelMod.Valueroot;
                row["IsOpen"] = DataConverter.CLng(labelMod.IsOpen);
                row["FalseContent"] = labelMod.FalseContent;
                row["EmptyContent"] = labelMod.EmptyContent;
                row["DataSourceType"] = labelMod.DataSourceType;
                row["ConnectString"] = labelMod.ConnectString;
                row["ProceName"] = labelMod.ProceName;
                row["ProceParam"] = labelMod.ProceParam;
                table.Rows.Add(row);
            }
            return table;
        }
        ...
        public DataTable SelAllLabelWithTime(string labelCate, string labelName)
        {
            DataTable dt = SelAllLabel(labelCate,labelName);
            dt.Columns.Add(new DataColumn("CDate",typeof(DateTime)));
            dt.Columns.Add(new DataColumn("EDate",typeof(DateTime)));
            string dir = function.VToP(vdir);
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                DataRow dr = dt.Rows[i];
                string fileName = dir + dr["LabelCate"] + "\\" + dr["LabelName"] + ".label";
                if (!File.Exists(fileName)) { continue; }
                FileInfo file = new FileInfo(fileName);
                dr["CDate"] = file.CreationTime;
                dr["EDate"] = file.LastWriteTime > file.CreationTime ? file.LastWriteTime : file.CreationTime ;
            }
            return dt;
        }
        ...
        {
            return DBCenter.DB.Field_List(tbname);
        }
        /// <summary>
        /// 根据标签名称判断是否存在该标签
        /// </summary>
        ...
        public static string GetLabelVPath(M_Label label)
        {
            ...
            return labelBll.vdir + label.LabelCate + "/" + label.LabelName + ".label";
        }
        /// <summary>
        /// 验证目录是否存在
        /// </summary>
        ...
        {
            string lblLabels = "";
            //{0}:outtype,{1}:LabelName,{2}:Server.UrlEncode(labelinfo.LabelName),
            string labelTlp = "<div outtype='{0}' code='{1}' onclick='cit(this)' class='spanfixdivchechk text-left'>"
                + "<a onclick=opentitle('LabelSql?LabelName={2}','修改标签') href='javascript:;' title='修改标签'><span class='fa fa-edit'>"
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                {
                    case 1://静态标签
                        lblLabels = lblLabels + string.Format(labelTlp, 1, labelinfo.LabelName, HttpUtility.UrlEncode(labelinfo.LabelName));
                        break;
                    case 3://数据源
                        if (string.IsNullOrEmpty(labelinfo.Param))
                        {
                            lblLabels = lblLabels + string.Format(labelTlp, 3, labelinfo.LabelName, HttpUtility.UrlEncode(labelinfo.LabelName));
                        }
                        else
                        {
                            ...
                            if (Param.IndexOf("|") < 0)
                            {
                                ...
                                {
                                    //带参数数据源
                                    lblLabels = lblLabels + string.Format(labelTlp, 2, labelinfo.LabelName, HttpUtility.UrlEncode(labelinfo.LabelName));
                                }
                            }
                            else
                            {
                                ...
                                {
                                    lblLabels = lblLabels + string.Format(labelTlp, 4, labelinfo.LabelName, HttpUtility.UrlEncode(labelinfo.LabelName));
                                }
                            }
                        }
                        break;
                    case 5:
                        if (DataConverter.CLng(labelinfo.LabelType) == 5)
                        {
                            lblLabels = lblLabels + string.Format(labelTlp, 5, labelinfo.LabelName, HttpUtility.UrlEncode(labelinfo.LabelName));
                        }
                        else
                        {
                            lblLabels = lblLabels + string.Format(labelTlp, 6, labelinfo.LabelName, HttpUtility.UrlEncode(labelinfo.LabelName));
                        }
                        break;
                    default://动态标签
                        if (string.IsNullOrEmpty(labelinfo.Param))
