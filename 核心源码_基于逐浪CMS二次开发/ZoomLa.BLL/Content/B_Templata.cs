...
namespace ZoomLa.BLL
{
    ...
    public class B_Templata
    {
        ...
        {
            SafeSC.CheckIDSEx(ids);
            string sql = "UPDATE " + strTableName + " SET IsTrue=" + istrue + " WHERE  TemplateID IN (" + ids + ") AND Userid!=0";
            return SqlHelper.ExecuteSql(sql);
        }
        ...
        public PageSetting ReadUserall_Page(int cpage, int psize, int ParentID, int UserID)
        {
            string where = "(ParentID=@ParentID and userid=0)  or (userid=@Userid and ParentID=@ParentID)";
        ...
        public int GetOrderID(int id)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                return DataConvert.CLng(SqlHelper.ExecuteScalar(CommandType.Text, strSql3, null));
            }
            else
                return 0;
        }
