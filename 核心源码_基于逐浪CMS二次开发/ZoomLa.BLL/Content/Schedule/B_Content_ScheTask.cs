...
namespace ZoomLa.BLL
{
    public class B_Content_ScheTask
    {
        ...
        public PageSetting SelPage(int cpage, int psize,Com_Filter filter)
        {
            ...
            string where = "1=1 ";
            List<SqlParameter> sp = new List<SqlParameter>();
            if (!string.IsNullOrEmpty(filter.type) && !filter.type.Equals("-100")) { where += " AND TaskType=" + DataConvert.CLng(filter.type); }
            if (!string.IsNullOrEmpty(filter.skey)) { where += " AND TaskName LIKE @TaskName"; sp.Add(new SqlParameter("TaskName", "%" + filter.skey + "%")); }
