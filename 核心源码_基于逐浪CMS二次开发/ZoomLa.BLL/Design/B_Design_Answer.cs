...
namespace ZoomLa.BLL.Design
{
    public class B_Design_Answer
    {
        ...
        /// 返回指定IP最后一次的提交信息
        /// </summary>
        public M_Design_Answer SelModelByIP(string ip)
        {
            List<SqlParameter> sp = new List<SqlParameter>() { new SqlParameter("ip", ip) };
            using (DbDataReader reader = DBCenter.SelReturnReader(TbName, "ip=@ip", PK + " DESC", sp))
            {
                if (reader.Read())
                {
                    return initMod.GetModelFromReader(reader);
