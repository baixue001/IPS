...
namespace ZoomLa.BLL.Design
{
    public class B_Design_Scence : ZL_Bll_InterFace<M_Design_Page>
    {
        ...
        /// 根据场景状态查询,默认查询未删除的场景
        /// </summary>
        public PageSetting SelModelByStatus(int cpage, int psize, string skey, int status = -100)
        {
            ...
            fields += ",(SELECT COUNT(ID) FROM ZL_COM_VisitCount WHERE InfoID=CONVERT(VARCHAR(50),A.ID)) AS ViewCount";
            PageSetting setting = PageSetting.Double(cpage, psize, TbName, "ZL_User", "A.ID", "A.UserID = B.UserID", where, "A.ID DESC", sp, fields);
