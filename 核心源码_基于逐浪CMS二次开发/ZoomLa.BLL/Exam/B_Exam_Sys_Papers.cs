...
namespace ZoomLa.BLL
{
    public class B_Exam_Sys_Papers : ZL_Bll_InterFace<M_Exam_Sys_Papers>
    {
        ...
        #endregion
        public PageSetting SelPage(int cpage, int psize, int classid = -100, int uid = 0)
        {
            string where = "1=1 ";
            List<SqlParameter> sp = new List<SqlParameter>();
            if (classid != -100) { where += " AND (p_class=@p_class OR p_class IN ( SELECT C_id FROM ZL_Exam_Class WHERE C_Classid=@p_class ))"; sp.Add(new SqlParameter("p_class", classid)); }
            if (uid > 0) { where += " AND A.UserID=" + uid; }
            PageSetting setting = PageSetting.Double(cpage, psize, TbName, "ZL_Exam_Class", PK, "A.p_class=B.c_id", where, "", sp, "A.*,B.C_ClassName");
        ...
        {
            return Sql.Sel(TbName, "", "ID DESC");
        }
        public DataTable SelByIDS(string ids)
        {
            SafeSC.CheckIDSEx(ids);
            return SqlHelper.ExecuteTable("SELECT * FROM " + TbName + " WHERE ID IN(" + ids + ")");
        }
        /// <summary>
        /// 查询所有试卷(可按类型)
        /// </summary>
        ...
        public DataTable SelAll(int type = 0)
        {
            ...
            string sql = "SELECT * FROM " + TbName + " WHERE 1=1" + wherestr;
            return SqlHelper.ExecuteTable(sql);
        }
        public PageSetting SelAll_SPage(int cpage, int psize, int type = 0, string skey = "")
        {
            string where = "1=1";
            List<SqlParameter> sp = new List<SqlParameter>();
            if (type > 0) { where += " AND p_class=" + type; }
            if (!string.IsNullOrWhiteSpace(skey)) { where += " AND p_name LIKE @name"; sp.Add(new SqlParameter("name", "%" + skey + "%")); }
        ...
        //-----------------------
        public string GetPType(object o)
        {
            switch (DataConvert.CStr(o))
            {
                case "1":
                    return "考试";
                case "2":
