...
namespace ZoomLa.BLL
{
    ...
    public class B_Exam_Sys_Questions
    {
        ...
        {
            ids = StrHelper.PureIDSForDB(ids);
            if (string.IsNullOrEmpty(ids) || SafeSC.CheckIDS(ids)) return null;
            SafeSC.CheckIDSEx(ids);
            string sql = "SELECT " + fields + ",IsRight=0,Remark='' FROM " + strTableName + " WHERE p_id IN(" + ids + ") ";
            if (qtype != 99)//过滤大题
            {
        ...
        public PageSetting SelByIDS_SPage(int cpage, int psize, string ids, int qtype = 0)
        {
            ids = StrHelper.PureIDSForDB(ids);
            if (string.IsNullOrEmpty(ids) || !SafeSC.CheckIDS(ids)) { return null; }
            string where = "p_id IN (" + ids + ")";
            if (qtype > 0) { where += " AND p_Type=" + qtype; }
            PageSetting setting = PageSetting.Single(cpage, psize, strTableName, PK, where, "p_type");
            DBCenter.SelPage(setting);
            return setting;
        }
        ...
        /// <param name=""></param>
        /// <returns></returns>
        public PageSetting SelPageByFilter(int cpage, int psize, int classid = 0, int qtype = 99, int grade = 0, int diff = 0, int version = 0, string keyword = "", int issmall = -100)
        {
            string where = "1=1";
            List<SqlParameter> sp = new List<SqlParameter>();
            if (classid > 0) { where += " AND p_class=" + classid; }
            if (qtype != 99) { where += " AND p_type=" + qtype; }
            if (grade > 0) { where += " AND p_Views=" + grade; }
            if (diff > 0) { where += " AND p_Difficulty=" + diff; }
            if (version > 0) { where += " AND Version=" + version; }
            if (!string.IsNullOrWhiteSpace(keyword)) { where += " AND p_title LIKE @keyword"; sp.Add(new SqlParameter("keyword", "%" + keyword + "%")); }
            if (issmall != -100) { where += " AND IsSmall=" + issmall; }
            PageSetting setting = PageSetting.Single(cpage, psize, strTableName, PK, where, "p_CreateTime DESC", sp);
            DBCenter.SelPage(setting);
            return setting;
        }
