...
namespace ZoomLa.BLL
{
    ...
    public class B_UserCourse
    {
        ...
        /// <returns></returns>
        public int GetInsert(M_UserCourse model)
        {
        ...
        /// <param name="UserCourse"></param>
        /// <returns></returns>
        public bool GetUpdate(M_UserCourse model)
        {
        ...
        private static M_UserCourse GetInfoFromReader(DbDataReader rdr)
        {
        ...
        public PageSetting SelPage(int cpage, int psize, int userid = -100, int aunit = -100, int cid = -100)
        {
            string where = " 1=1";
            if (userid != -100) { where += " AND UserID=" + userid; }
            if (aunit != -100) { where += " AND Aunit=" + aunit; }
            if (cid != -100) { where += " AND CourseID IN ( SELECT id FROM zl_course WHERE CoureseClass =" + cid + ")"; }
            PageSetting setting = PageSetting.Single(cpage, psize, strTableName, PK, where, "Aunit,AddTime DESC");
