...
{
    //-----------------------Base64
    public static string Base64Encrypt(string str, Encoding encode = null)
    {
        if (encode == null) { encode = Encoding.UTF8; }
        byte[] bytes = encode.GetBytes(str);
        return Convert.ToBase64String(bytes);
    }
    public static string Base64Decrypt(string str, Encoding encode = null)
    {
    ...
    public static string AESDecrypt(string cryedText, byte[] key = null)
    {
        ...
        cs.Close();
        ms.Close();
        return System.Text.Encoding.UTF8.GetString(decryptBytes).Replace("\0", ""); 
    }
