...
namespace ZoomLa.BLL.Helper
{
    public class ImageDealLib
    {
        ...
        public static string imgcrop(string picpath, string spath, int x1, int y1, int width, int height, FileCache filecache, out string warning)
        {
            ...
            Image _sourceimg_common = null;
            Bitmap _currimg_common = null;
            Graphics _g_common = null;

            if (checkfile == true)
            {
                //从源文件创建imgage
                Image temp = Image.FromFile(_sourceimg_common_mappath);
                _sourceimg_common = new Bitmap(temp);
                temp.Dispose();
                ...
                //设置画笔
                //_g_common.CompositingMode = Drawing2D.CompositingMode.SourceOver;
                //_g_common.InterpolationMode = Drawing2D.InterpolationMode.HighQualityBicubic;
                //_g_common.PixelOffsetMode = Drawing2D.PixelOffsetMode.Half;
                ...
                //_currimg_common.Save(_spath_common_mappath, Imaging.ImageFormat.Jpeg);
                ...
                if (filecachecode == 1)
                {
                    File.Delete(_sourceimg_common_mappath);
                }
        ...
        public static string makewatermark(string picpath, string waterspath, WaterType watermodel, string spath, ImageType imgtype, FileCache filecache, out string warning, int trans = 100)
        {
            ...
            {
                //从指定源文件,创建image对象
                _sourceimg_water = Image.FromFile(_sourceimg_water_mappath);
            }
            else
            {
        ...
        public static string makewatermark(string picpath, Image water, WaterType watermodel, string spath, ImageType imgtype, FileCache filecache, out string warning, int trans = 100)
        {
            #region
            //反馈信息
            StringBuilder checkmessage = new StringBuilder();
            ...
            {
                //从指定源文件,创建image对象
                _sourceimg_common = Image.FromFile(_sourceimg_common_mappath);
            }
            else
            {
            ...
            if (string.IsNullOrEmpty(checkmessage.ToString()))
            {
                ...
                //从源图创建画板
                Graphics _g_common = Graphics.FromImage(_sourceimg_common);
                ...
                //switch (imgtype)
                //{
                //    case ImageType.JPEG: _sourceimg_common.Save(_spath_common_mappath, Imaging.ImageFormat.Jpeg); break;
                //    case ImageType.GIF: _sourceimg_common.Save(_spath_common_mappath, Imaging.ImageFormat.Gif); break;
                //    case ImageType.PNG: _sourceimg_common.Save(_spath_common_mappath, Imaging.ImageFormat.Png); break;
                //}
        ...
        public static string Resizepic(string picpath, ResizeType model, string spath, ImageType imgtype, double? mwidth, double? mheight, FileCache filecache, out string warning)
        {
            ...
            if (string.IsNullOrEmpty(checkmessage.ToString()))
            {
                ...
                switch (imgtype)
                {
                    //case ImageType.JPEG: _currimg_common.Save(_spath_common_mappath, Imaging.ImageFormat.Jpeg); break;
                    //case ImageType.GIF: _currimg_common.Save(_spath_common_mappath, Imaging.ImageFormat.Gif); break;
                    //case ImageType.PNG: _currimg_common.Save(_spath_common_mappath, Imaging.ImageFormat.Png); break;
                }
