...
namespace ZoomLa.BLL
{
    public class RegexHelper
    {
        ...
        public static List<string> GetValuesByWhere(string content, string schar, string echar)
        {
            List<string> result = new List<string>();
            if (StrHelper.StrNullCheck(content, schar, echar)) { return result; }
            while (content.IndexOf(schar) > -1 && content.IndexOf(echar) > -1)
            {
                int start = content.IndexOf(schar);
                int end = content.IndexOf(echar, start + schar.Length);
                int len = ((end + echar.Length) - start);
                result.Add(content.Substring(start, len));
                content = content.Remove(start, len);
            }
            return result;
        }
        /// <summary>
