    ...
    public class StrHelper
    {
        ...
        public static string RemoveBySE(string msg, string schar = "#", string echar = "#")
        {
            while (msg.Contains(schar) && msg.Contains(echar))
            {
                ...
                if (end < 0) { break; }
                else { msg = msg.Remove(start, (end - start) + 1); }
            }
            return msg;
        }
        /// <summary>
        ...
        public static string DecompressString(string str)
        {
            ...
            catch (Exception ex) { throw new Exception(ex.Message + "|||" + str); }
        }
        #endregion
        #region 网址相关
        ...
        /// <summary>
        /// 使用:Url_AddParam(Request.RawUrl,"NodeID=5","Skey=50");
        /// </summary>
        public static string Url_AddParam(string url, params string[] myparams)
        {
            ...
            foreach (string param in myparams)
            {
                string key = param.Split('=')[0].ToLower();
                string value = param.Split('=')[1];
            ...
            {
                url_base += d.Key + "=" + d.Value + "&";
            }
            return url_base.TrimEnd('&').TrimEnd('?');
        }
        #endregion
        #region IDS
        /// <summary>
        ...
        public static string AddToIDS(string ids, string[] b)
        {
            ...
            {
                if (string.IsNullOrEmpty(b[i])) {continue; }
        ...
        /// IDS合并,并去除重复
        /// </summary>
        public static string IDS_Merge(string idsa, string idsb)
        {
            return AddToIDS(idsa, idsb.Split(",".ToCharArray(), StringSplitOptions.RemoveEmptyEntries));
        }
        /// <summary>
        /// 对比新旧IDS,获取两者之间不同的部分(add|remove)
        /// </summary>
        /// <returns>add|remove,为空则没有新增与移除</returns>
        public static string IDS_GetChange(string oldids, string newids)
        {
            string add = "", remove = "";
            if ((oldids ?? "").Equals((newids ?? ""))) { return ""; }
            if (string.IsNullOrEmpty(oldids)) { return newids + "|"; }
            if (string.IsNullOrEmpty(newids)) { return "|" + oldids; }
            string[] oldarr = oldids.Split(",".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
            string[] newarr = newids.Split(",".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
            //是否新ids存在旧ids不存在(add)
            foreach (string id in newarr)
            {
                if (oldarr.FirstOrDefault(p => p.Equals(id)) == null) { add += id + ","; }
            }
            //是否旧ids存在,新ids不存在(remove)
            foreach (string id in oldarr)
            {
                if (newarr.FirstOrDefault(p => p.Equals(id)) == null) { remove += id + ","; }
            }
            return add.Trim(',') + "|" + remove.Trim(',');
        }
        /// <summary>
        /// 从ids中移除指定id(贪婪模式)
        /// </summary>
        ...
        public static string ToIDS(DataTable dt, string field = "id")
        {
            string ids = "";
            if (dt == null || dt.Rows.Count < 1) { return ids; }
            foreach (DataRow dr in dt.Rows)
            {
                ids += dr[field].ToString() + ",";
            }
            return ids.Trim(',');
        }
        //-----数据库相关
        ...
        /// MobileMsg
        /// </summary>
        public static string IDSToLikeSql(string ids,string field)
        {
            string sql = "";
            ...
            {
                string id = idArr[i];
                if (i == 0) { sql += " "+ field + " LIKE '%," + id + ",%' "; }
                else { sql += " OR "+ field + " LIKE '%," + id + ",%' "; }
            }
            return string.IsNullOrEmpty(sql) ? "" : "(" + sql + ")";
        }
        /// <summary>
        /// 从DataTable中取出指定字段,并以,号切割返回,前后不加,
        /// </summary>
        public static string GetIDSFromDT(DataTable dt, string field)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                if (string.IsNullOrEmpty(DataConvert.CStr(dr[field]))) { continue; }
                result += dr[field] + ",";
            }
            return result.Trim(',');
        }
        ...
        /// <summary>
        /// SQL排序,必须指定allowField,field_desc
        /// </summary>
        /// <param name="order">排序字符串</param>
        /// <param name="allowFields">允许的字段以,切割</param>
        /// <returns></returns>
        public static string SQL_OrderBy(string order, string allowFields, string defaultOrder = "")
        {
            if (string.IsNullOrEmpty(order) || !order.Contains("_")) { return defaultOrder; }
            //未指定为为避免安全问题,直接忽略
            if (string.IsNullOrEmpty(allowFields)) { return ""; }
            //-------------------------------------------
            string[] fields = allowFields.ToLower().Split(',');
            string field = order.Split('_')[0].ToLower();
            string sort = order.Split('_')[1].ToUpper();
            if (!sort.Equals("ASC")) { sort = "DESC"; }
            string value = fields.FirstOrDefault(m => m.Equals(field));
            if (string.IsNullOrEmpty(value)) { return ""; }
            else { return (" " + field + " " + sort); }//UserID DESC
        }
        //-----数组相关
        /// <summary>
        /// 数组合并
