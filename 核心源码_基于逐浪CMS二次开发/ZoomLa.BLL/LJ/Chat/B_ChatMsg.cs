...
public class B_ChatMsg
{
    ...
    public string GetOnlineJson(bool only_visitor)
    {
        ...
        foreach (M_OnlineUser user in OnlineUserList)
        {
            if (only_visitor && !user.IsVisitor) { continue; }
            if (user.UserID == ignore) { continue; }//不返回自身
    ...
    /// 以发起人和接收人归并信息,
    /// </summary>
    /// <param name="uids">发起人的UserIDS,游客是四位英文字母</param>
    /// <returns></returns>
    public DataTable SelBySR(string uids)
    {
        ...
        List<SqlParameter> sp = new List<SqlParameter>();
        string insql = "";
        string[] uidArr = uids.Split(",".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
        for (int i = 0; i < uidArr.Length; i++)
        {
            sp.Add(new SqlParameter("uid" + i, uidArr[i]));
            insql += "@uid" + i + ",";
        }
        insql = insql.TrimEnd(',');
        string sql = "SELECT * FROM " + TbName + " WHERE ID IN (SELECT min(ID) FROM " + TbName + " WHERE UserID IN(" + insql + ") GROUP BY UserID,ReceUser)";
