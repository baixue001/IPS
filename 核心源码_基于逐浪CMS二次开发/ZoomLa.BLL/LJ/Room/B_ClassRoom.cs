...
namespace ZoomLa.BLL
{
    ...
    public class B_ClassRoom
    {
        ...
        public PageSetting SelPage_All(int cpage, int psize, int userid = 0, int status = -1, string skey = "")
        {
            ...
            if (userid > 0) { where += " AND CreateUser=" + userid; }
            if (status > -1) { where += " AND IsTrue =" + status; }
            if (!string.IsNullOrEmpty(skey)) { where += " AND RoomName LIKE @name"; sp.Add(new SqlParameter("name", "%" + skey + "%")); }
            //该班级成员人数字段（按成员类别统计）
