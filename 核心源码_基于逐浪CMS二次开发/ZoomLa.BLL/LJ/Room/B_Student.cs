...
namespace ZoomLa.BLL
{
    ...
    public class B_Student
    {
        ...
        public PageSetting SelByURid_SPage(int cpage, int psize, int roomid, int auditing = -1, int stutype = 0, int userid = 0)
        {
            string where = "RoomID=" + roomid;
            if (auditing > -1) { where += " AND Auditing=" + auditing; }
            if (stutype > 0) { where += " AND StudentType=" + stutype; }
