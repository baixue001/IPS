...
namespace ZoomLa.BLL.Message
{
    ...
    public class B_BaikeEdit
    {
        ...
        {
            return SelPage(1, int.MaxValue, new Com_Filter() { uids = uid.ToString(), status = status.ToString() }).dt;
        }
        //, int uid, int status
        public PageSetting SelPage(int cpage, int psize,Com_Filter filter)
        {
            string where = "1=1 ";
            List<SqlParameter> sp = new List<SqlParameter>();
            if (!string.IsNullOrEmpty(filter.uids)) { where += " AND UserID IN ("+filter.uids+")"; }
