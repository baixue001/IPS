...
namespace ZoomLa.BLL
{
    public class B_Guest_Bar : ZL_Bll_InterFace<M_Guest_Bar>
    {
        ...
        public PageSetting SelPage(int cpage, int psize, int cateid, int uid, string key, bool isaudit = false)
        {
            ...
            //匿名用户不提供搜索
            if (uid > 0) { where += " AND CUser=" + uid; }
            if (!string.IsNullOrEmpty(key)) { where += " AND Title LIKE @skey"; }
            if (isaudit)
            {
                where += " AND Status>0 And Status IS NOT NULL";
            }
            string order = "OrderFlag DESC,R_CDate DESC,CDate DESC";
            List<SqlParameter> sp = new List<SqlParameter>() { new SqlParameter("skey", "%" + key + "%") };
        ...
        /// 查找指定贴子与贴子的回复
        /// </summary>
        public PageSetting SelByID(int cpage, int psize, M_Guest_Bar barMod, string filter)
        {
            string fields = "A.*,B.salt AS UserFace,B.HoneyName";
            string where = "A.ID=" + barMod.ID + " OR A.Pid=" + barMod.ID + " AND A.Status>0";
            if (!string.IsNullOrEmpty(filter))
            {
                //只显示层主
                if (filter.Contains("OnlyLayer")) { where += " AND CUser=" + barMod.CUser; }
            }
            string order = "A.PID,A.ID ASC";
            PageSetting setting = PageSetting.Double(cpage, psize, TbName, "ZL_User", "A.ID", "A.CUser=B.UserID", where, order, null, fields);
        ...
        /// 贴子的回复
        /// </summary>
        public PageSetting SelByPid(int cpage, int psize, int pid)
        {
            string fields = "A.*,B.salt AS UserFace,B.HoneyName";
        ...
        public DataTable SelByCateIDS(string ids, string key = "", int status = -10)
        {
            ...
            if (!string.IsNullOrEmpty(key)) { wherestr += " And Title Like @key"; }
            //不包含回收站   
            if (status == -10) { wherestr += " AND A.[Status]NOT IN (" + (int)ZLEnum.ConStatus.Recycle + ")"; }
            else
            {
        ...
        public bool UpdateStatus(int cateid, string ids, int status, int uid = 0)
        {
            ...
            if (uid != 0) { where += " AND CUser=" + uid; }
            return DBCenter.UpdateSQL(TbName, "Status=" + status, where);
        }
        /// <summary>
        /// 主题数,回贴数
        /// </summary>
