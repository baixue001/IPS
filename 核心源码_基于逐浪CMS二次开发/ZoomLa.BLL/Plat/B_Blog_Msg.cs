...
namespace ZoomLa.BLL.Plat
{
    public class B_Blog_Msg : ZL_Bll_InterFace<M_Blog_Msg>
    {
        ...
        public void AddRead(string ids)
        {
            ...
            {
                paraIds += GetParentIds(DataConvert.CLng(id));
            }
            paraIds = paraIds.Trim(',');
            if (!string.IsNullOrEmpty(paraIds))
            {
                DBCenter.UpdateSQL(TbName, "ReadNumber+=1", "ID IN (" + paraIds + ")");
            }
        }
        /// <summary>
        /// 获取包括自身的所有父级ID，以 , 分割
        /// </summary>
        private string GetParentIds(int id)
        {
            string fids = "";
            M_Blog_Msg msgMod = SelReturnModel(id);
            if (msgMod != null)
            {
                fids += msgMod.ID + ",";
                if (msgMod.ForWardID > 0 && msgMod.ForWardID == msgMod.ID) { fids += GetParentIds(msgMod.ForWardID); }
            }
            return fids;
        }
