...
namespace ZoomLa.BLL.Plat
{
    public class B_User_Token:ZL_Bll_InterFace<M_User_Token>
    {
        ...
        {
            using (DbDataReader dr = Sql.SelReturnReader(TbName, PK, ID))
            {
                if (dr.Read())
                {
        ...
        {
            using (DbDataReader dr = Sql.SelReturnReader(TbName, " Where Uid=" + uid))
            {
                if (dr.Read())
                {
                    return iniModel.GetModelFromReader(dr);
        ...
        public M_User_Token SelByOpenID(string openID, string account = "QQ")
        {
            ...
            using (DbDataReader dr = Sql.SelReturnReader(TbName, " WHERE [" + account + "]=@openid", sp))
            {
                if (dr.Read())
                {
                    return iniModel.GetModelFromReader(dr);
