...
namespace ZoomLa.BLL
{
    public class B_Questions_Knowledge
    {
        ...
        public PageSetting SelPage(int cpage, int psize, int cid = -100, string name = "")
        {
            ...
            PageSetting setting = PageSetting.Single(cpage, psize, strTableName, PK, where, "CDate DESC", sp);
            setting.fields = "*,(SELECT COUNT(*) FROM " + strTableName + " WHERE Pid = A.k_id) AS ChildCount,"
                            + "(SELECT GradeName FROM ZL_Grade WHERE A.Grade = GradeID) AS GradeName,"
                            + "(SELECT C_ClassName FROM ZL_Exam_Class WHERE A.k_Class_id = C_id) AS C_ClassName,"
                            + "(SELECT UserName FROM ZL_User WHERE A.CUser = UserID) AS UserName";
