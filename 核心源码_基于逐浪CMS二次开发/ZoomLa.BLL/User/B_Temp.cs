...
namespace ZoomLa.BLL.User
{
    public class B_Temp : ZoomLa.BLL.ZL_Bll_InterFace<M_Temp>
    {
        ...
        {
            using (DbDataReader reader = Sql.SelReturnReader(TbName, " WHERE UserID=" + uid + " AND UseType=" + usetype))
            {
                if (reader.Read())
                {
                    return initMod.GetModelFromReader(reader);
        ...
        public M_Temp SelModelByStr1(string str1)
        {
            ...
            using (DbDataReader reader = Sql.SelReturnReader(TbName, " WHERE str1=@str1", sp))
            {
                if (reader.Read())
                {
                    return initMod.GetModelFromReader(reader);
