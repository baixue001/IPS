...
namespace ZoomLa.BLL
{
    ...
    public class B_UserPromotions
    {
        ...
        public bool GetInsertOrUpdate(M_UserPromotions userpromotions)
        {
            ...
            SqlParameter[] cmdParams = userpromotions.GetParameters();
            int Pcount = DataConvert.CLng(SqlHelper.ExecuteScalar(CommandType.Text, sqls, cmdParams));
            ...
            {
                string sqlStr = "INSERT INTO [dbo].[ZL_UserPromotions] ([GroupID],[NodeID],[look],[addTo],[Modify],[Columns],[Comments],[Deleted],[quote],[down]) VALUES (@GroupID,@NodeID,@look,@addTo,@Modify,@Columns,@Comments,@Deleted,@quote,@down);select @@IDENTITY";
                return (DataConvert.CLng(SqlHelper.ExecuteScalar(CommandType.Text, sqlStr, cmdParams)) > 0);
            }
        }
        /// <summary>
        /// 用户组在指定节点的权限模型
        ...
        {
            string result = "";
            string sql = "SELECT STR(NodeID,4)+',' FROM ZL_UserPromotions  Where GroupID=" + gid + " And addTo>0 FOR XML PATH('')";//将多行组合成一句，返回
            DataTable dt=SqlHelper.ExecuteTable(CommandType.Text,sql);
            if (dt != null && dt.Rows.Count > 0)
                result = dt.Rows[0][0].ToString().Replace(" ","");
            return result;
        }
