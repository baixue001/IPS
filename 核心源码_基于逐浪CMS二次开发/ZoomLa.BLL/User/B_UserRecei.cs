...
namespace ZoomLa.BLL
{
    ...
    public class B_UserRecei
    {
        ...
        {
            string sqlStr = "SELECT * FROM [dbo].[ZL_UserRecei] WHERE [ID] = "+UserReceiID;
        ...
        public M_UserRecei GetSelect(int id, int uid)
        {
            ...
            using (DbDataReader reader = Sql.SelReturnReader(strTableName, sql))
            {
                if (reader.Read())
                {
                    return initMod.GetModelFromReader(reader);
        ...
        public PageSetting SelByUid_SPage(int cpage,int psize,int uid)
        {
            string where = "UserID="+uid;
            PageSetting setting = PageSetting.Single(cpage,psize,strTableName,PK,where,"IsDefault DESC");
            DBCenter.SelPage(setting);
            return setting;
        }
        ...
        public int SelecCount(int userid)
        {
            ...
            return DataConvert.CLng(SqlHelper.ExecuteScalar(CommandType.Text, sqlstr, para));
        }
        /// <summary>
        /// 设置为默认地址
        /// </summary>
