...
namespace ZoomLa.BLL.User
{
    public class B_User_API
    {
        ...
        /// </summary>
        public static M_UserInfo GetLogin(HttpContext ctx, string openid)
        {
            //未输入OpenID则读取网站用户(微信公众号)
            if (string.IsNullOrEmpty(openid)) { return new B_User(ctx).GetLogin(); }
            //------------APP使用
        ...
        {
            using (DbDataReader reader = Sql.SelReturnReader(TbName, " WHERE " + where, sp))
            {
                if (reader.Read())
                {
