...
namespace ZoomLa.BLL
{
    public class B_User_Plat
    {
        ...
        {
            using (DbDataReader reader = Sql.SelReturnReader(TbView, PK, ID))
            {
                if (reader.Read())
                {
        ...
        {
            using (DbDataReader reader = Sql.SelReturnReader(TbView, " Where UserID=" + id + " And CompID=" + compid))
            {
                if (reader.Read())
                {
                    return initMod.GetModelFromReader(reader);
