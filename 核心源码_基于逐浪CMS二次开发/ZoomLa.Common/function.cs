namespace ZoomLa.Common
{
    ...
    public static class function
    {
        ...
        public static string GetRandomString(int length, int type = 1)
        {
            ...
            switch (type)
            {
                ...
                case 4://验证码使用
                    ...
                    charPool = "ABCDEFGHJKLMNPRSTUVWXYZ234578";//soi1069gpq
                    break;
                default:
        ...
        {
            return function.GetRandomString(10) + "@random.com";
        }
        /// <summary>
        /// 产生随机数
        /// </summary>
        ...
        {
            return ZoomLa.Safe.IOPath.PToV(ppath);
        }
        /// <summary>
        /// 虚拟路径转物理路径,用于异步等地方
        /// </summary>
        ...
        {
            return ZoomLa.Safe.IOPath.VToP(vpath);
        }
        /// <summary>
        /// 获取上传图片的imgurl
        /// </summary>
        ...
        {
            if (string.IsNullOrEmpty(DataConverter.CStr(o))) { return "/UploadFiles/nopic.svg"; }
            string url = o.ToString().Trim().ToLower();
            string updir = "/UploadFiles/";
            //-------------------------------------------------
            if (url.IndexOf("://") > 0 || url.StartsWith("/"))//不做处理
            {

            }
            else if (url.IndexOf("uploadfiles/") < 0 && url.IndexOf(updir) < 0)
            {
                //如不带uploadfiles目录则补上,否则只补上最前一斜杠
                url = ("/" + (updir + url).TrimStart('/')).Replace("//", "/");
            }
            return url;
        }
        //-----------数组，字符串
        ...
        public static void WriteErrMsg(string msg,string url="") { throw new Exception(msg); }
    }
}