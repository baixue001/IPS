...
namespace ZoomLa.Model
{
    ...
    public abstract class M_Base
    {
        ...
        protected DateTime ConvertToDate(object o)
        {
            ...
            if (o != null && o != DBNull.Value)
            {
                string str = o.ToString().Replace("T", " ");//json格式处理
                DateTime.TryParse(str, out date);
            }
            return date;
        }
        protected string PureIdsForDB(string ids)
        {
            ids = ids ?? "";
            if (string.IsNullOrEmpty(ids) || ids.Equals(",")) { return ""; }
        ...
        /// <summary>
        /// 用于Title等前端填写,但不允许有html和符号的字段
        /// </summary>
        protected string HtmlEncode(string str) { return System.Web.HttpUtility.HtmlEncode(str); }
        //-----------------------------------------仅用于升级工具
        string up_tables = "";
        //相关联的表,用于一个模型对应多表的状况
        [JsonIgnore]
        public virtual string UP_Tables { get { return up_tables; } }
    }
}
