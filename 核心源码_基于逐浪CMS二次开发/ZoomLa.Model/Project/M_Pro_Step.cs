...
namespace ZoomLa.Model.Project
{
    public class M_Pro_Step : M_Base
    {
        ...
        {
            M_Pro_Step model = this;
            if (model.CDate <= DateTime.MinValue) { model.CDate = DateTime.Now; }
            if (model.EDate <= DateTime.MinValue) { model.EDate = DateTime.Now; }
            ...
            sp[12].Value = model.Partic;
            sp[13].Value = model.EDate;
            return sp;
        }
        public M_Pro_Step GetModelFromReader(DbDataReader rdr)
        {
            ...
            model.Partic = ConverToStr(rdr["Partic"]);
