...
namespace ZoomLa.SQLDAL.SQL
{
    public class MSSqlHelper : SqlBase
    {
        ...
        /// 用于执行Insert,返回ID
        /// </summary>
        public override object ExecuteScalar(SqlModel model)
        {
            string sql = GetSQL(model);
            using (SqlConnection conn = GetConnection(model))
            {
        ...
        public override DataTable ExecuteTable(SqlModel model)
        {
            ...
            using (SqlConnection conn = GetConnection(model))
            {
                ...
                try
                {
                    ...
                    {
                        return dataSet.Tables["Result"];//"Result"
                    }
                    return null;
                }
        ...
        {
            return DataConvert.CLng(ExecuteScalar(model));
        }
        //--------------分页与连结
        public override DataTable SelPage(PageSetting config)
        {
            ...
            else
            {
                ...
                config.countsql = "SELECT COUNT(*) FROM " + config.t1 + " A WHERE " + config.where;
            }
        ...
        public DataTable SelPageByRow(PageSetting config)
        {
            ...
            config.pageCount = GetPageCount(config.itemCount, config.psize);
            if (config.debug) { throw new Exception(config.sql); }
            return ExecuteTable(new SqlModel() { sql = config.sql, spList = config.spList });
        }
        ...
        public override bool Table_Add(string table, M_SQL_Field field)
        {
            ...
            if (field.ispk) { fieldstr += " IDENTITY (1, 1) PRIMARY Key NOT NULL"; }
            else if (!string.IsNullOrEmpty(field.defval)) { fieldstr += " DEFAULT ('" + field.defval + "')"; }
           
            ExecuteNonQuery(new SqlModel() { sql = string.Format(sql, fieldstr) });
            return true;
        }
        ...
        public override DataTable View_List()
        {
            return DBHelper.View_List();
        }
