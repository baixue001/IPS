...
namespace ZoomLa.SQLDAL.SQL
{
    ...
    public class MySqlHelper : SqlBase
    {
        ...
        public override string GetSQL(SqlModel model)
        {
            ...
            switch (model.cmd)
            {
                case SqlModel.SqlCmd.Insert:
                    ...
                    sql = "INSERT INTO " + model.tbName + " ({0}) VALUES({1});SELECT LAST_INSERT_ID();";
        ...
        public override bool Table_Exist(string name)
        {
            ...
            {
                if (dr[0].ToString().ToLower().Equals(name)) { return true; }
            }
            return false;
        }
        ...
        public override DataTable Table_List()
        {
            string sql = "SHOW TABLES;";
            DataTable dt= ExecuteTable(new SqlModel() { sql = sql });
            dt.Columns[0].ColumnName = "Name";//Tables_in_xxxx
            return dt;
        }
        ...
        {
            string sql = "DESCRIBE " + table;//勿使用中文空格
            DataTable dt= ExecuteTable(new SqlModel() { sql = sql });
            dt.Columns["Field"].ColumnName = "Name";
            return dt;
        }
