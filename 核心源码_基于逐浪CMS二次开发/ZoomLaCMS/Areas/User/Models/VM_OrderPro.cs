...
namespace ZoomLaCMS.Areas.User.Models
{
    public class VM_OrderPro
    {
        ...
        public VM_OrderPro(M_UserInfo mu, HttpRequest Request) 
        {
            this.SType = DataConvert.CLng(Request.GetParam("SType"));
            this.mu = mu;
            this.OrderNo = Request.GetParam("OrderNo");
            switch (SType)
            {
                case 0://订单
                    orderMod = orderBll.SelModelByOrderNo(OrderNo);
                    if (orderMod == null || orderMod.id == 0) throw new Exception("订单不存在");
                    if (orderMod.Userid != mu.UserID) throw new Exception("该订单不属于你,无法查看");
