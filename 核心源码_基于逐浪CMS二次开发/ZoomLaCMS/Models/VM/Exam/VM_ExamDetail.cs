...
namespace ZoomLaCMS.Models.Exam
{
    ...
    public class VM_ExamDetail
    {
        ...
        public VM_ExamDetail(M_UserInfo mu, M_Exam_Sys_Papers paperMod, HttpRequest Request)
        {
            this.mu = mu;
            this.paperMod = paperMod;
            Action = (Request.GetParam("action")).ToLower();
            FlowID = Request.GetParam("FlowID");
            ...
            if (Action.Equals("view"))//显示答案
            {
                ...
                if (AnswerDT.Rows.Count < 1) { throw new Exception("你尚未完成答卷"); }
        ...
        //显示回答区
        public IHtmlContent GetSubmit(DataRow dr)
        {
            return MvcHtmlString.Create(questBll.GetSubmit(DataConverter.CLng(dr["p_id"]), DataConverter.CLng(dr["p_Type"]), ref AngularJS));
        }
        //该试题是否已被批改
        public IHtmlContent GetIsRight(DataRow dr)
        {
            string tlp = "";
            switch (dr["IsRight"].ToString())
            {
                case "0":
                    tlp = "<span class='zi zi_cloud' style='font-size:1.2em;' title='尚未批阅'></span><span></span>";
                    break;
                case "1":
                    tlp = "<span class='zi zi_check' style='font-size:1.2em;color:green;' title='正确'></span><span>(" + dr["Score"] + "分)</span>";
                    break;
                case "2":
                    tlp = "<span class='zi zi_times' style='font-size:1.2em;color:red;' title='错误'></span>";
                    break;
                default:
                    break;
            }
