...
function LeftSwitchFunc() {
    ...
    if (openleft) {
        ...
        $("#leftSwitch .fa").attr("class","fa fa-chevron-circle-right");
    }
    else {
        ...
        $("#leftSwitch .fa").attr("class", "fa fa-chevron-circle-left");
    }
    $("#leftSwitch").show();
}
//读取配置参数初始化左边栏
...
function ShowMain(leftUrl, rightUrl, obj, remind) {//优先加载右边iframe
    ...
    //}, 100);
    setTimeout(function () {
        if (!leftUrl || leftUrl == "" || leftUrl == lastLeft) { return; }
        $("#left_Hid").val(leftUrl);
        lastLeft = leftUrl;
        $.post("/Admin/Index/ASCXLoad", { "name": leftUrl }, function (data) {
            myFrame.AddTabJson(currentFrameTabId, leftUrl);
            $("#left_Div").html(data); $("#left_Div").show();
            //<script>ascxInit('" + ascxName + "');ClearSpin();DivCache('" + ascxName.ToLower() + "');setLayout();</script>"
            ascxInit(leftUrl); ClearSpin();
            DivCache(leftUrl.toLowerCase());
            setLayout();
            //关键词记忆
            setTimeout(function () {
                myFrame.SetKeyword(currentFrameTabId); $(".ascx_key").bind("blur paste clip", function () { myFrame.SaveKeyword(currentFrameTabId, $(this).val()); });
