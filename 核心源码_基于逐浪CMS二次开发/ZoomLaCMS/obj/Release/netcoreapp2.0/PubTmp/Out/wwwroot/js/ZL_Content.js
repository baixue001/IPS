...
function DupTitleFunc(title) {
    ...
    $.post("/Admin/Content/Content_API", { action: "duptitle", value: title }, function (data) {
...
function ShowCutImg(param) {
    ShowDiag("/Plugins/PicEdit/CutPic?" + param, "图片编辑");
}
...
function SortImg(pval) {
    if ($("#txt_" + pval.inputid).val().trim() != "") {
        ...
        ShowDiag("/Common/Dialog/GroupImgs?json={\"imgs\":" + txtval + ",\"pval\":" + JSON.stringify(pval) + "}", "请拖动图片进行排序");
    } else {
        alert("请选择或上传图片!");
    };
}
...
conhelp.verbak.showDiag = function () {
    ShowDiag("/Admin/Content/Addon_VerBak?GeneralID=" + zlconfig.gid, "存档备份 <span class='rd_green'>(版本历史存于ZL_Content_VerBak表中)</span>");
}