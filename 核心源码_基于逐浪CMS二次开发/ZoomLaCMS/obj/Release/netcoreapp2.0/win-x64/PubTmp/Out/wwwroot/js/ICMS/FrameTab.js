...
function SwitchIframe(iFrameTab) {
    ...
    if (newMainFrame.length <= 0) {//是否新建标签
        newMainFrame = $("#main_right_frame").prepend($("#iframeMainTemplate").html())
				.find("[tabid=0]").attr({ "tabid": tabId, "src": "Main", "id": "main_right", "name": "main_right" })
				.css("display", "block");
    } else {
...
function InitNewFrameTab() {
    $("#newFrameTab").click(function () {
        ...
        $('<li id="iFrameTab' + (++lastFrameTabId) + '" ><a href="javascript:"><span id="frameTabTitle">新选项卡</span><a class="closeTab" title="关闭"><span class="zi zi_times"></span></a></a></li>').insertBefore(this).iFrameTab();
        frameTabCount++;
