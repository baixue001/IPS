...
function LeftSwitchFunc() {
    ...
    if (openleft) {
        ...
        //$("#leftSwitch img").attr("src", "/images/Admin/butOpen.gif");
        ...
        //$("#leftSwitch img").attr("src", "/images/Admin/butClose.gif");
...
function ShowMain(leftUrl, rightUrl, obj,remind) {//优先加载右边iframe
    if (rightUrl && rightUrl != "") { 
        ...
        temp.src = "/Html/wait.html?ver=5" + (remind ? "&r=" + encodeURI(remind) : "");
    ...
    setTimeout(function () {
        if (leftUrl && leftUrl != "" && $("#left_Hid").val() !== leftUrl) {
            ...
            $.post("/Admin/Index/ASCXLoad", { "name": leftUrl }, function (data) {
                ...
                $("#left_Div").html(data);
                ascxInit(leftUrl); ClearSpin(); DivCache(leftUrl.toLowerCase()); setLayout();
            })
