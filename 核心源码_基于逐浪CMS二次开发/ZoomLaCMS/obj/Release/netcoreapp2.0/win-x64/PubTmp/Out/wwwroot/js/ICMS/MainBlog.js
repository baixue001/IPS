...
function PreView(vpath) {
    ...
    if (vpath != $("#preview_down_a").attr("href"))//如果预览的文件变更，则重新加载
    {
        ...
        $("#preview_if").attr("src", "/PreView?vpath=" + escape(vpath));
        $("#largepre_a").attr('href', "/PreView?vpath=" + escape(vpath));
    }
}
function LoadReply(pid, pageSize, pageIndex) {
    $("#reply_" + pid).load("/Plat/Blog/ReplyList?code=" + Math.random() + "&pid=" + pid + "&PageSize=" + pageSize + "&PageIndex=" + pageIndex + " start");
}
...
function ShowGroupAt() {
    ShowComDiag("/Plat/common/selgroup?source=plat#atuser", "请选择需要@的部门或联系人");
}
function AddTopic() {
...
function CollFunc(obj, id)//收藏,取消收藏
{
    ...
    {
        $(obj).attr("class", "zi zi_love nocolled");
        PostToCS("ReColl", id, function () { });
    }
    else//加入收藏
    {
        $(obj).attr("class", "zi zi_loves colled");
...
function PostToCS(a, v, CallBack) {
    PostToCS2("/Plat/Blog/Default", a, v, CallBack);
}//Post To CS end;
