//博客签到 签到
var sign = { api: "/Plat/Common/Signin" };
//初始化签到挂件
sign.init = function () {
...
function PrivateSend() {
    ...
    $.post("/Plat/Common/Common", msgjson, function (data) { PrivateCallBack(data); }, "json");
}
//发送成功或失败后,启用发送按钮
...
function PrivateSelUser(select) {
    $("#User_IFrame").attr("src", "/Plat/Common/SelUser?Type=AllInfo#" + select);
...
function ChatShow(id, uname) {
    ...
    window.open("/Common/Chat/?uid=" + id);
}
function ChatClose() {
    $("#chatdiv").hide();
}
...
function PreView(vpath) {
    ...
    if (vpath != $("#preview_down_a").attr("href"))//如果预览的文件变更，则重新加载
    {
        ...
        $("#preview_if").attr("src", "/Com/PreView?vpath=" + escape(vpath));
        $("#largepre_a").attr('href', "/Com/PreView?vpath=" + escape(vpath));
    }
}
function LoadReply(pid, psize, cpage) {
    $("#reply_" + pid).load("/Plat/Blog/ReplyList?code=" + Math.random() + "&pid=" + pid + "&psize=" + psize + "&cpage=" + cpage + " start");
}
...
function AddVoteOP() {
    ...
    var tr = "<tr class='votetr'><td></td><td><label class='vote_op_label'>" + index + ".</label><input type='text' name='VoteOption_T' class='vote_op_input form-control' /><span class='zi zi_times' onclick='RemoveVoteOP();'></span></td></tr>";
...
function ShowGroupAt() {
    ShowComDiag("/Plat/common/SelUser?source=plat#atuser", "请选择需要@的部门或联系人");
}
function AddTopic() {
...
function CollFunc(obj, id)//收藏,取消收藏
{
    ...
    {
        $(obj).attr("class", "zi zi_starLine nocolled");
        PostToCS("ReColl", id, function () { });
    }
    else//加入收藏
    {
        $(obj).attr("class", "zi zi_star colled");
...
function PostToCS(a, v, CallBack) {
    PostToCS2("/Plat/Blog/Blog_API", a, v, CallBack);
}//Post To CS end;
...
function ShowFileUP() {
    ...
    attachDiag.url = "/IO/WebUP?json={\"ashx\":\"action=Blog\",\"pval\":\"\"}";
...
datef.tlp = "<div id=\"datef_div\">"
            ...
            + "<input type=\"text\" class=\"form-control mt-1\" id=\"datef_etime_t\" placeholder=\"结束时间\" onclick=\"WdatePicker({ dateFmt: 'yyyy/MM/dd' })\" />"
            + "<div class=\"text-center mt-1\">"
            + "<input type=\"button\" value=\"确定\" style='width:48%;' class=\"btn btn-outline-info\" onclick=\"datef.filter($('#custom_li'),'customer');\" />"
            + "<input type=\"button\" value=\"取消\" style='width:48%;' class=\"btn btn-outline-info\" onclick=\"datef.close();\" />"
            + "</div>"
            + "</div>"
            + "</div>";
datef.$pop = $(datef.tlp)
...
pop.bindEvent = function ($items) {
    $items.popover({
        animation: true, placement: 'right', title: "<i class=\"zi zi_useralt\"></i> 用户信息<a href=\"javascript:;\" class='pull-right' title='关闭' onclick=\"$('.uinfo').popover('hide');\"><i class='zi zi_times'></i></a>", content: function () {
        ...
        + "<ul class=\"uinfoul\">"
        + "<li class=\"\">"+datas.UserName
        + "<input type=\"button\" class=\"btn btn-sm btn-outline-info ml-1\"  value=\"AT他\" onclick=\"AddAT('"+datas.UserName+"', '"+datas.UserID+"');\" />"
        + "</li>"
        ...
        + "<a href=\"/Plat/Blog/?uids="+datas.UserID+"\" class=\"btn btn-sm btn-outline-info\">工作流</a>"
        + "<a href=\"/Plat/Blog/?uids="+datas.UserID+"&view=timeline\" class=\"btn btn-sm btn-outline-info\">时间线</a>"
        + "<input type=\"button\" class=\"btn btn-sm btn-outline-info\" value=\"私信\" onclick=\"ChatShow('"+datas.UserID+"', '"+datas.UserName+"');\" />"
        + "<input type=\"button\" class=\"btn btn-sm btn-outline-info\" value=\"站内邮\" onclick=\"PrivateOpen('"+datas.UserID+"', '"+datas.UserName+"'); $('.uinfo').popover('hide');\" />"
        + "</div>"
        + "</div>"
        + "</div>"
            
            return datatlp;
            //return '<div><i class="zi zi_spinner zi_pulse" style="font-size:3em;"></i></div>';
...
pop.tlp = "<div class=\"uinfodiv\">"
        ...
        + "<input type=\"button\" class=\"btn btn-sm btn-outline-info\" style=\"margin-left:8px;\" value=\"AT他\" onclick=\"AddAT('@UserName', '@UserID');\" />"
        + "</li>"
        ...
        + "</div>"
        + "<div class=\"uinfo_bottom\">"
        + "<a href=\"/Plat/Blog/?uids=@UserID\" class=\"btn btn-sm btn-outline-info\">工作流</a>"
        + "<a href=\"/Plat/Blog/?uids=@UserID&view=timeline\" class=\"btn btn-sm btn-outline-info\">时间线</a>"
        + "<input type=\"button\" class=\"btn btn-sm btn-outline-info\" value=\"私信\" onclick=\"ChatShow('@UserID', '@UserName');\" />"
        + "<input type=\"button\" class=\"btn btn-sm btn-outline-info\" value=\"站内邮\" onclick=\"PrivateOpen('@UserID', '@UserName'); $('.uinfo').popover('hide');\" />"
        + "</div>"
        + "</div>"
        + "</div>"
//-----------------------------
